...
(function () {
	...
	function pager(meta, hash) {
		...
		if (p > 1) h += '<a class="button" href="#' + hash + sep + 'page=' + (p - 1) + '">← ' + esc(__('Prev')) + '</a> ';
		h += '<span class="wh-page">' + esc(__('Page')) + ' ' + p;
		if (meta.total != null) h += ' · ' + fmtNum(meta.total) + ' ' + esc(__('total')) + ' (' + esc(meta.total_mode || '') + ')';
		h += '</span> ';
		if (meta.has_more) h += '<a class="button" href="#' + hash + sep + 'page=' + (p + 1) + '">' + esc(__('Next')) + ' →</a>';
		return h + '</nav>';
	}
	function parseHash() {
	...
	function nav() {
		...
		NAV.forEach(function (it) {
			h += '<li><a href="#' + it[0] + '"' + (r === it[0] ? ' aria-current="page" class="current"' : '') + '>' + esc(labels[it[0]]) + '</a></li>';
		});
		return h + '</ul><button class="button wh-refresh" id="wh-refresh" type="button">' + esc(__('Refresh metadata')) + '</button></nav><div id="wh-view" aria-live="polite"></div>';
	}
	...
	function viewOverview(el) {
		...
		Promise.all([api('database'), api('cron').catch(function () { return { data: null }; })]).then(function (all) {
			...
			var h = '<h2 class="wh-title">' + esc(__('Database overview')) + '</h2><section class="wh-grid">';
			...
			h += '</ul><p><a class="button" href="#health">' + esc(__('Open diagnostics')) + '</a></p></div>';
			...
			if (cron) {
				...
				h += '<div class="wh-card"><h3>' + esc(__('Scheduled')) + '</h3><p class="wh-stat">' + fmtNum(cron.wp_cron.overdue) + '</p><p class="wh-stat-sub">' + esc(__('overdue events')) + '</p><ul class="wh-list">'
					...
					+ '<p><a class="button" href="#health">' + esc(__('Open diagnostics')) + '</a></p></div>';
			}
			}
	...
	function viewTables(el, params) {
		...
		getOwners(function (owners) {
			...
			api(q).then(function (res) {
				...
				var h = '<h2 class="wh-title">' + esc(__('Tables')) + '</h2>';
				h += '<form class="wh-filters" id="wh-tfilter"><label>' + esc(__('Search')) + ' <input type="search" name="s" value="' + esc(params.search || '') + '"></label> '
					...
					+ '<button class="button" type="submit">' + esc(__('Filter')) + '</button></form>';
				if (filtered) {
	...
	function viewTableDetail(el, name, params) {
		...
		api('tables/' + encodeURIComponent(name)).then(function (res) {
			...
			h += '<div class="wh-tabs" role="tablist" aria-label="' + esc(__('Table sections')) + '">'
				+ '<a class="button" role="tab" aria-selected="' + (tab === 'struct') + '" href="' + base + '">' + esc(__('Structure')) + '</a> '
				+ '<a class="button" role="tab" aria-selected="' + (tab === 'rows') + '" href="' + base + '?tab=rows">' + esc(__('Data')) + '</a> '
				+ '<a class="button" role="tab" aria-selected="' + (tab === 'rels') + '" href="' + base + '?tab=rels">' + esc(__('Relationships')) + '</a></div><div id="wh-tabbody"></div>';
			el.innerHTML = h;
			focusHeading(el);
	...
	function renderDataTab(body, d, params) {
		...
		api(url).then(function (r2) {
			...
			var h = '<form class="wh-filters" id="wh-dsearch"><label>' + esc(__('Search rows')) + ' <input type="search" name="q" value="' + esc(params.s || '') + '" minlength="2" maxlength="100"></label> '
				+ '<button class="button" type="submit">' + esc(__('Search')) + '</button> '
				+ (r2.data.length ? '<button class="button" type="button" id="wh-csv">' + esc(__('Export CSV')) + '</button>' : '') + '</form>';
			...
			h += '<div class="wh-scroll"><table class="widefat striped"><thead><tr>' + cols.map(function (c) { return '<th scope="col">' + esc(c) + '</th>'; }).join('') + (canInspect ? '<th scope="col"><span class="screen-reader-text">' + esc(__('Actions')) + '</span></th>' : '') + '</tr></thead><tbody>'
				+ r2.data.map(function (row) {
					...
					if (canInspect) cells += '<td><button type="button" class="button button-small wh-rowbtn" data-id="' + esc(row[addr.columns[0]]) + '">' + esc(__('View')) + '</button></td>';
					return '<tr>' + cells + '</tr>';
	...
	function openRowModal(table, pkCol, id) {
		...
		api('tables/' + encodeURIComponent(table) + '/rows/' + encodeURIComponent(id)).then(function (res) {
			...
			var h = '<h2 id="wh-rowtitle">' + esc(table) + ' <span class="wh-mode">(' + esc(pkCol) + ' = ' + esc(id) + ')</span></h2><dl>'
				...
				+ '</dl><p><button type="button" class="button" id="wh-modalclose">' + esc(__('Close')) + '</button></p>';
			modal.innerHTML = h;
			modal.querySelector('#wh-modalclose').addEventListener('click', close);
			modal.querySelector('#wh-modalclose').focus();
		}).catch(function (e) { modal.innerHTML = errorState(e.message) + '<p><button type="button" class="button" id="wh-modalclose">' + esc(__('Close')) + '</button></p>'; modal.querySelector('#wh-modalclose').addEventListener('click', close); });
	}

	/* ---------- search ---------- */
	function viewSearch(el, params) {
		el.innerHTML = '<h2 class="wh-title">' + esc(__('Global search')) + '</h2>'
			+ '<form class="wh-filters" id="wh-sform"><label>' + esc(__('Search database')) + ' <input type="search" name="q" value="' + esc(params.q || '') + '" minlength="2" maxlength="100" required></label> '
			...
			+ '<button class="button button-primary" type="submit">' + esc(__('Search')) + '</button></form><div id="wh-sres"></div>';
		focusHeading(el);
	...
	function viewHealth(el, params) {
		el.innerHTML = '<h2 class="wh-title">' + esc(__('Diagnostics')) + '</h2><div id="wh-hbody">' + loading() + '</div>';
		focusHeading(el);
	...
	function viewQuery(el) {
		el.innerHTML = '<h2 class="wh-title">' + esc(__('Query console')) + '</h2>'
			+ '<div class="wh-notice" role="note">' + esc(__('Read-only console. Only SELECT, SHOW, DESCRIBE and EXPLAIN run — the server rejects everything else.')) + '</div>'
			...
			+ '<p><button class="button button-primary" type="submit">' + esc(__('Run')) + '</button> <button class="button" type="button" id="wh-explain">' + esc(__('Explain')) + '</button></p></form><div id="wh-qres"></div><div class="wh-history" id="wh-qhist"></div>';
		focusHeading(el);
		...
		function renderHistory() {
			...
			box.innerHTML = '<h3>' + esc(__('Recent queries (this browser only)')) + '</h3><ul>' + items.map(function (s) {
				return '<li><button type="button" data-sql="' + esc(s) + '">' + esc(s.length > 90 ? s.slice(0, 90) + '…' : s) + '</button></li>';
			}).join('') + '</ul>';
	...
	/* ---------- map ---------- */
	function viewMap(el) {
		el.innerHTML = '<h2 class="wh-title">' + esc(__('Database map')) + '</h2>' + loading(__('Building relationship graph…'));
		focusHeading(el);
		api('map').then(function (res) {
			var g = res.data, groups = {};
			g.nodes.forEach(function (n) { (groups[n.classification] = groups[n.classification] || []).push(n); });
			var bySource = {};
			g.edges.forEach(function (e) { (bySource[e.source_table] = bySource[e.source_table] || []).push(e); });
			var h = '<p class="wh-result-meta">' + g.nodes.length + ' ' + esc(__('tables')) + ', ' + g.edges.length + ' ' + esc(__('relationship(s)')) + (g.truncated ? ' — ' + esc(__('edges truncated at the safety cap')) : '') + '. '
				+ badge('PHYSICAL', 'rel-physical') + ' ' + esc(__('declared constraint')) + ' · ' + badge('INFERRED', 'rel-inferred') + ' ' + esc(__('evidence-based guess, never a constraint')) + '</p>';
			Object.keys(groups).sort().forEach(function (cls) {
				h += '<div class="wh-card" style="margin-block-end:12px"><h3>' + badge(cls, cls.toLowerCase()) + ' (' + groups[cls].length + ')</h3><ul class="wh-list">';
				groups[cls].forEach(function (n) {
					var edges = bySource[n.id] || [];
					h += '<li><a href="#tables/' + esc(n.id) + '">' + esc(n.id) + '</a>';
					if (edges.length) {
						h += '<ul>' + edges.slice(0, 10).map(function (e) {
							return '<li>' + badge(e.origin, 'rel-' + e.origin.toLowerCase()) + ' <code>' + esc(e.source_column) + '</code>' + (e.target_table ? ' → ' + esc(e.target_table) : '') + '</li>';
						}).join('') + (edges.length > 10 ? '<li>…+' + (edges.length - 10) + '</li>' : '') + '</ul>';
					} else {
						h += ' <span class="wh-mode">— ' + esc(__('no known relationships')) + '</span>';
					}
					h += '</li>';
				});
	...
	function viewAudit(el, params) {
		el.innerHTML = '<h2 class="wh-title">' + esc(__('Audit log')) + '</h2>' + loading();
		focusHeading(el);
		api('audit?page=' + encodeURIComponent(params.page || 1)).then(function (res) {
			if (!res.data.length) { el.innerHTML = '<h2 class="wh-title">' + esc(__('Audit log')) + '</h2>' + emptyState(__('No audit events recorded yet.')); return; }
			el.innerHTML = '<h2 class="wh-title">' + esc(__('Audit log')) + '</h2><div class="wh-scroll"><table class="widefat striped"><thead><tr><th scope="col">' + esc(__('Time')) + '</th><th scope="col">' + esc(__('Event')) + '</th><th scope="col">' + esc(__('Actor')) + '</th><th scope="col">' + esc(__('Target')) + '</th><th scope="col">' + esc(__('Outcome')) + '</th></tr></thead><tbody>'
	...
	function viewSettings(el) {
		el.innerHTML = '<h2 class="wh-title">' + esc(__('Settings')) + '</h2>' + loading();
		focusHeading(el);
		Promise.all([api('settings'), api('context')]).then(function (all) {
			var s = all[0].data, ctx = all[1].data;
			var h = '<section class="wh-grid"><form id="wh-setform" class="wh-card"><h3>' + esc(__('Tuning')) + '</h3>'
				+ '<p><label>' + esc(__('Cache TTL (seconds, 60–3600)')) + '<br><input type="number" name="cache_ttl" min="60" max="3600" value="' + esc(s.cache_ttl) + '"></label></p>'
				+ '<p><label>' + esc(__('Default page size (5–100)')) + '<br><input type="number" name="default_per_page" min="5" max="100" value="' + esc(s.default_per_page) + '"></label></p>'
				+ '<p><label>' + esc(__('Max page size (10–200)')) + '<br><input type="number" name="max_per_page" min="10" max="200" value="' + esc(s.max_per_page) + '"></label></p>'
				+ '<p><label>' + esc(__('Max query rows (50–2000)')) + '<br><input type="number" name="max_query_rows" min="50" max="2000" value="' + esc(s.max_query_rows) + '"></label></p>'
				+ '<p><button class="button button-primary" type="submit">' + esc(__('Save')) + '</button></p><div id="wh-setmsg"></div></form>'
				+ '<div class="wh-card"><h3>' + esc(__('Environment')) + '</h3><dl class="wh-kv">'
				+ '<dt>' + esc(__('Multisite')) + '</dt><dd>' + (ctx.is_multisite ? esc(__('Yes')) : esc(__('No'))) + '</dd>'
				+ '<dt>' + esc(__('Site ID')) + '</dt><dd>' + esc(ctx.blog_id) + '</dd>'
				+ '<dt>' + esc(__('Table prefix')) + '</dt><dd><code>' + esc(ctx.prefix) + '</code></dd>'
				+ '<dt>' + esc(__('Version')) + '</dt><dd>' + esc(s.version) + '</dd></dl></div>'
				+ '<div class="wh-card wh-wide"><h3>' + esc(__('Capabilities')) + '</h3><ul class="wh-list">' + Object.keys(s.capabilities || {}).map(function (k) { return '<li><code>' + esc(k) + '</code> — ' + esc(s.capabilities[k]) + '</li>'; }).join('') + '</ul></div></section>';
			el.innerHTML = h;
			el.querySelector('#wh-setform').addEventListener('submit', function (ev) {
				ev.preventDefault();
	...
	function viewSnapshots(el) {
		el.innerHTML = '<h2 class="wh-title">' + esc(__('Snapshots')) + '</h2>'
			+ '<div class="wh-notice" role="note">' + esc(__('Snapshots freeze database structure (never row content) so two points in time can be compared.')) + '</div>'
			+ '<form class="wh-filters" id="wh-snapform"><label>' + esc(__('Label (optional)')) + ' <input type="text" name="label" maxlength="191"></label> '
			+ '<button class="button button-primary" type="submit">' + esc(__('Create snapshot')) + '</button></form>'
			...
			+ '<button class="button" type="submit">' + esc(__('Import')) + '</button></form>'
				...
				var h = '<div class="wh-scroll"><table class="widefat striped"><thead><tr><th scope="col">' + esc(__('Label')) + '</th><th scope="col">' + esc(__('Created')) + '</th><th scope="col">' + esc(__('Tables')) + '</th><th scope="col">' + esc(__('Actions')) + '</th></tr></thead><tbody>'
					+ res.data.map(function (s) {
						return '<tr><td>' + esc(s.label) + '</td><td>' + esc(s.created_at) + '</td><td>' + fmtNum(s.tables) + '</td>'
							+ '<td><button type="button" class="button button-small" data-dl="' + esc(s.id) + '">' + esc(__('Download')) + '</button> '
							+ '<button type="button" class="button button-small" data-del="' + esc(s.id) + '">' + esc(__('Delete')) + '</button></td></tr>';
					}).join('') + '</tbody></table></div>';
				h += '<form class="wh-filters" id="wh-difform"><label>A <select name="a">' + opts + '</select></label> '
					+ '<label>B <select name="b">' + opts + '</select></label> '
					+ '<button class="button button-primary" type="submit">' + esc(__('Compare')) + '</button></form>';
				box.innerHTML = h;
